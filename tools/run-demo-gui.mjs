// BlockCanvas · 自动化建站、导出并保留界面供交互检查
// 运行流程：
//   1. 启动真实 Electron 图形窗口
//   2. 自动载入使用智能关系选择器的落地页 SceneGraph
//   3. 自动触发导出到 E:\Develop\demo-page.html
//   4. 运行完成后【保持程序界面完全打开】，供你在屏幕上直接操作、点击元素、审查关系选择器与外观！
...
const ROOT = 'E:/Develop/BlockCanvas';
const EXPORT_PATH = 'E:/Develop/demo-page.html';
...
async function main() {
  console.log('🚀 正在启动 BlockCanvas 桌面客户端...');
  const app = await electron.launch({
    args: ['.'],
    cwd: ROOT,
    executablePath: electronPath,
    env: { ...process.env, BC_EXPORT_PATH: EXPORT_PATH }
  });

  const win = await app.firstWindow();
  await win.waitForSelector('.app', { state: 'visible', timeout: 15000 });
  await win.setViewportSize({ width: 1400, height: 920 });

  console.log('🎨 正在载入自动化构建的现代网页场景...');
  await win.evaluate((scene) => {
    if (window.__tabStore) {
      const ts = window.__tabStore.getState();
      ts.newTab('现代产品落地页', scene, 'E:/Develop/BlockCanvas/data/projects/demo-page.bcproj');
    } else if (window.__sceneStore) {
      window.__sceneStore.getState().setScene(scene);
    }
  }, demoScene);
  ...
  console.log('====================================================');
  console.log('✨ 自动化构建与导出完成！');
  console.log('📄 导出的网页文件:', EXPORT_PATH);
  console.log('🖥️ 【程序界面保持打开】你可以在弹出的客户端窗口中直接交互、点选元素！');
  console.log('====================================================');

  // 保持程序界面打开，监听窗口关闭事件
  await app.waitForEvent('close');
  console.log('用户已关闭窗口，脚本正常退出。');
  process.exit(0);
}
