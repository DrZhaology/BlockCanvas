...
- **快捷助手**：把「多条 CSS 属性配合才能做出的效果」打包成一句话 —— Flex / Grid 布局、文字渐变；生成的属性可一键从下方 CSS 列表隐藏（按选择器记忆）
...
- **设备三合一**：电脑 / 平板 / 手机 = 画布宽度 = 编辑断点，画布顶部常驻设备切换条（未选中元素也可切换）
- **编辑器技巧提示**：画布左上角每 30 秒轮播一条使用技巧（含 `Shift + /` 呼出全部快捷键）
